...
<html>
  <head><title>NVCL Download Services :: WFS Download Page</title></head>
	...
	<tr><td>
		<c:choose>
			<c:when test="${msgMap['status'] == 'success'}">
				...
				you do not receive an email you can manually check the status of your request <a href='<c:out value="${msgMap['webappURL']}"/>checkwfsstatus.html?email=
				<c:out value="${msgMap['email']}"/>'>here</a>
			</c:when>
			<c:otherwise>
				You will need to manually check the status of your request <a href='<c:out value="${msgMap['webappURL']}"/>checkwfsstatus.html?email=
				<c:out value="${msgMap['email']}"/>'>here</a>
				</c:otherwise>
				</c:choose>
